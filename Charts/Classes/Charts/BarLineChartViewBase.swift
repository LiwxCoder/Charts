...
public class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    private var _autoScaleLastLowestVisibleXIndex: Double?
    private var _autoScaleLastHighestVisibleXIndex: Double?
    ...
    {
        return abs(highestVisibleXIndex - lowestVisibleXIndex)
    }
    ...
    {
        let valsInView = getDeltaY(axis) / Double(_viewPortHandler.scaleY)
        
        let job = MoveChartViewJob(
            viewPortHandler: viewPortHandler,
            xValue: 0.0,
            yValue: yValue + valsInView / 2.0,
            transformer: getTransformer(axis),
            view: self)
    ...
    {
        let valsInView = getDeltaY(axis) / Double(_viewPortHandler.scaleY)
        let xsInView = xAxis.axisRange / Double(_viewPortHandler.scaleX)
        
        let job = MoveChartViewJob(
            viewPortHandler: viewPortHandler,
            xValue: xValue - xsInView / 2.0,
            yValue: yValue + valsInView / 2.0,
            transformer: getTransformer(axis),
            view: self)
    ...
    public var lowestVisibleXIndex: Double
    {
        var pt = CGPoint(
    ...
    public var highestVisibleXIndex: Double
    {
        var pt = CGPoint(
